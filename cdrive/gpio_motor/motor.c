...
#define DEVICE_NODE_NAME  "chrdev_motor"   // 设备节点名称

// GPIO寄存器的物理地址定义
#define GPIO_DR 0xfe200000 
#define GPIO_H  0xfe20001c	//GPSET GPIO输出控制寄存器地址
#define GPIO_L  0xfe200028 //GPCLR GPIO清除寄存器地址

// 定义虚拟地址指针
unsigned int *vir_gpio5_dr = NULL;
unsigned int *vir_gpio5_h = NULL;
unsigned int *vir_gpio5_l = NULL;
...
static int chrdev_init(void)
{
    ...
    // 对物理地址进行虚拟地址映射
    vir_gpio5_dr = ioremap(GPIO_DR, 4);
    if (vir_gpio5_dr == NULL)
    {
        printk("gpio5dr ioremap error\n");
        return -EBUSY;
    }

    vir_gpio5_h = ioremap(GPIO_H, 4);
    if (vir_gpio5_h == NULL)
    {
        printk("gpio5h ioremap error\n");
        return -EBUSY;
    }

    vir_gpio5_l = ioremap(GPIO_L, 4);
    if (vir_gpio5_l == NULL)
    {
        printk("gpio5l ioremap error\n");
        return -EBUSY;
    }
